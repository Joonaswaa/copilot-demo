...
| 10 | **RPA Automation Center** | One-click weekly workflow: ERP detection → validation → analytics → reports → email draft → archive |

## RPA / Workflow Automation Layer

The app simulates a weekly supply chain automation process — not just a dashboard,
but an end-to-end **RPA-style workflow**:

```
ERP export detection → validation → analytics → purchase recommendations
  → management reports (PDF / PowerPoint) → email draft → archive
```
...
```
data/
  erp_exports/          ← drop your CSV/Excel ERP export here
  processed/            ← archived ERP files after successful runs
reports/
  weekly/               ← generated PDF, PPTX, Markdown reports
  archive/<run_id>/     ← copy of all outputs per run
outbox/
  purchase_recommendations/   ← CSV purchase proposals
  email_drafts/               ← HTML email drafts (not sent automatically)
logs/
  workflow_runs/        ← JSON log per workflow run
```

### How to run the workflow

1. Copy your ERP export (`.csv`, `.xlsx`, or `.xls`) into `data/erp_exports/`
   (a sample file is included on first setup).
2. Start the app: `streamlit run app.py`
3. Open **Automation Center** in the top navigation.
4. Click **Run Weekly RPA Workflow**.
5. Review the step log, download output files, and inspect `outbox/` and `reports/weekly/`.

The workflow runs fully **offline** — no SAP, SMTP, LLM, or cloud services required.
Optional email sending remains on the AI weekly report page.
...
├── app.py                     # Streamlit dashboard (8 pages + Copilot panel)
├── requirements.txt
├── README.md
├── .env.example               # config template (LLM keys, SMTP) — optional
├── data/
│   ├── sample_telecom_supply_chain_data.csv
│   ├── erp_exports/           # RPA: drop ERP exports here
│   └── processed/             # RPA: archived ERP files
├── reports/
│   ├── weekly/                # RPA: generated reports
│   └── archive/               # RPA: per-run archives
├── outbox/
│   ├── purchase_recommendations/
│   └── email_drafts/
├── logs/
│   └── workflow_runs/         # RPA: JSON run logs
├── outputs/
│   └── reports/               # legacy PDF output from dashboard
└── src/
    ├── data_generator.py      # synthetic ERP export generator
    ├── data_loader.py         # CSV/Excel loading + column validation
    ├── data_validation.py     # row-level ERP validation for RPA
    ...
    ├── pptx_exporter.py       # weekly management deck (EN/FI)
    ├── email_sender.py        # optional SMTP sending
    ├── rpa_workflow.py        # weekly RPA automation orchestrator
    └── kpi.py                 # shared KPI mask helpers
```
